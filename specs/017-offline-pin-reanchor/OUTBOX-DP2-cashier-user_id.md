...
1. **(RESOLVED) Cashier-PIN provisioning INSERT — built by POS-019 (merged, PR #398).** At the time this OUTBOX was written there was no `INSERT INTO cashier_pin_records` in POS `src/` (create path deferred from the 004 MVP). POS-019 has since built the born-neutral provisioning create path **keyed on `user_id` from creation**, plus the additive `user_id` column as migration `0035`. **Consequence:** 017's re-key migration (now numbered **`0036`** — `0035` is taken by 019's column) is a safety net for any **legacy** rows only; new rows are born neutral.
2. ~~Widen the `roster-handler.ts:43` allowlist to thread `user_id` through.~~ **CORRECTED (2026-06-14) — NO allowlist change needed or wanted.** POS-019 already built the full main-side consumption seam: `interpretRosterResponse` (`backend-client.ts`) populates the optional `user_id` from the wire, and `provisionCashierPin` resolves it against the roster main-side. The renderer-facing `roster-handler.ts` allowlist (`{id,display_name,role}`) **MUST stay strict** — widening it would leak the neutral key across the bridge (Constitution VII; the clerk↔neutral mapping is deliberately main-side). The seam lights up automatically now that DP-2 #573 shipped the field. (Verified: `pin-management.provision.test.ts` covers the populated-`user_id` success + pre-DP-2 `not_ready` paths, 71/71 green.)
