...
- `specs/017-offline-pin-reanchor` — POS Offline-PIN Re-Anchor off a DP-2 provider-neutral identifier (drift **D6**). **DESIGN-READY, IMPLEMENTATION-BLOCKED 2026-06-13 (PR #385, `a6d20bc`).** Re-keys `cashier_pin_records`'s composite PRIMARY KEY from the provider-coupled `cashier_clerk_user_id` onto the §16 provider-neutral `user_id`; PIN secret never moves; `clerk_user_id` → nullable bridge column. Full speckit chain authored: `/speckit-plan` ✅ (design-complete SQLite PK-rebuild; Constitution v1.5.1 check PASS, VIII advanced) · `/speckit-tasks` ✅ (20 tasks, **all `[BLOCKED: user_id-delivery]`**) · `/speckit-analyze` ✅ (0 CRITICAL, 100% coverage, `implementationBlocked: true`). **BLOCKER (verified on DP-2 `origin/main`, see [`BLOCKER.md`](specs/017-offline-pin-reanchor/BLOCKER.md)):** the provider-neutral `user_id` is **never delivered to the terminal** — shipped `PosOperatorSessionSummary` = `{ id, issued_at, envelope }` where `envelope` is opaque and `id` is the Clerk subject (`clerk_user_id`); zero `user_id` in any POS-facing contract; **D3** server-side `external_identity_links` exists (DP-2 #550) but resolves server-side only and its provisioning is deferred (029). 016 (D5) delivers the opaque envelope + `clerk_user_id`, NOT `user_id`. **Unblock:** a new DP-2 slice must surface `user_id` on a POS-facing response (drift-map NEW EDGE) + G10 ratification; then the design here executes as-is. **No migration SQL authored; `cashier_pin_records` untouched.**
