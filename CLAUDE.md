...
**§A5 status (2026-05-30):** **Resolved/done:** **T522** scrubber coverage ✅ (PR #299); **T524/T525** runbook + rollback docs ✅ (PR #308); **T521** runtime-redaction assertion ✅ done (PR #309) — its support-bundle half is N/A (no such tool exists yet), pending owner acceptance of N/A-by-absence (not self-ticked). **Owner hardware-target decision (2026-05-30, Ahmed):** 008 §A5 is **printer-only / OS-print / BIXOLON SRP-330 II (Option A)**; cash-drawer/DK1 hardware validation **deferred** to a future peripheral spec (drawer code stays); ESC/POS **descoped**; scanner = wedge-input observed/tested only. This **re-scopes** (does not close) T520a + T523 to printer-only — both **still OPEN** pending the owner "bar question" (does the bench smoke satisfy the perf bar + tested-row, or is a ≥20-run p95 capture still required?). **Still open / owner-gated:** T512 (`/impeccable` craft), T520a (printer OS-print perf, re-scoped), T523 (printer tested-row, re-scoped), T526 (security-review handoff), T529 (sign-off). See [`a5-verification-findings.md`](specs/008-sale-finalization-and-receipts/a5-verification-findings.md) + coordination.md §"Owner decision — 008 §A5 hardware target (2026-05-30)".
