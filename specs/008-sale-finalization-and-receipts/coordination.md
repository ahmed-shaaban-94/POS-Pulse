...
2. **Component ships; AppShell mount (T291) deferred on the missing recent-sale feed (v1-subset gap — owner review at PR).** Two feeds matter here, and the gap is sharper than first framed:
   ...
   - **✅ RESOLVED 2026-05-29 (Ahmed): DEFER T291; build the push primitive NEXT SLICE.** The S3c component shipped as-is (PR #281 `9fbb906`). T291 (AppShell mount) + the live banner feed land in a follow-up slice that first builds the renderer push primitive: `sales.subscribe(recent|banner_state)` graduates from the `not_implemented` stub to a real `webContents.send` + subscription-token registry, then AppShell subscribes to `banner_state`, sources `printFailure` from the projected state, and mounts `<PrinterFailureBanner>` (+ its sibling `<DrawerFailureBanner>` from Slice 4 once that lands). No fake null-fed mount in the interim.
