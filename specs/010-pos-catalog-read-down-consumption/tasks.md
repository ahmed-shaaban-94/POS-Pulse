...
- [ ] T001 Backend-contract coordination (non-code): finalize the catalogue-snapshot operation with the backend team against `specs/010-pos-catalog-read-down-consumption/contracts/backend-catalogue-snapshot.md`; obtain endpoint/verb, response shape, tenant/branch scoping, and terminal-auth mechanism (body-attestation vs header). **Clears §A6 (part 1).**
...
- [ ] T021 [BLOCKED:§A6] GREEN [US1] `createReadDownClient({ baseUrl, fetch, timeoutMs })` — GET the snapshot, attach device-token body-attestation from `secretStore` (per §A6 contract), map to typed outcome; token never bridged/logged — `src/main/catalogue/read-down/read-down-client.ts`
...
- [ ] T024 [US1] GREEN: `createReadDownWriter({ db, syncStateRepo })` — stage validated rows (fold via 009 `normalize()`), promote in one transaction (`DELETE` live + `INSERT … SELECT` from staging), write `last_success_at` in the same tx — `src/main/catalogue/read-down/read-down-writer.ts`
...
- [ ] T027 [US1] GREEN: enforce tenant-scoping in the writer (stage + promote filter the terminal's tenant; reject cross-tenant rows) — `src/main/catalogue/read-down/read-down-writer.ts`
...
- [ ] T032 [US2] GREEN: harden the promote — single transaction, rollback-on-throw, staging cleared per run so a prior failed run never leaks into a later promote — `src/main/catalogue/read-down/read-down-writer.ts`
...
- [ ] T039 [BLOCKED:§A6] [US2] Wire the driver into the composition root: instantiate `createReadDownClient` + `createReadDownWriter` + `createReadDownDriver`; run once on app-start / post-pairing; `start()` the interval; call `stop()` in `closeDbHandle()` **before** `dbHandle.close()` (mirror `finalize-listener` wiring) — `src/main/index.ts`
...
- [ ] T040 [US3] RED: bridge contract test — `catalogue:refresh` → `started` / `already_running` / `refused(no_session|tenant_isolation)`; `catalogue:freshness` → `ok{last_success_at|null}` / `refused`; both gate on session first; neither returns data/secret — `src/main/catalogue/__tests__/catalogue-bridge.readdown.test.ts`
- [ ] T041 [US3] GREEN: add `CATALOGUE_IPC_CHANNELS.REFRESH` + `FRESHNESS` and the request/response types — `src/shared/catalogue/channels.ts` + `src/shared/bridge-api.ts`
- [ ] T042 [US3] GREEN: extend `createCatalogueBridge` with `refresh` (session-gated → `driver.runTickOnce()`, single-flight status) and `freshness` (session-gated → `syncStateRepo` read; tenant-scoped) — `src/main/catalogue/catalogue-bridge.ts`
- [ ] T043 [US3] GREEN: register the two new channels — `src/main/ipc/catalogue.ts` (+ wire in `src/main/index.ts`)
- [ ] T044 [US3] GREEN: expose `catalogue.refresh` / `catalogue.freshness` on the preload surface — `src/preload/*` (the typed `catalogue.*` exposure)
- [ ] T045 [P] [US3] RED: freshness-indicator a11y/render test — shows "catalogue last updated &lt;time&gt;" (or "never updated" when null); no live-sync wording; keyboard-operable refresh affordance; axe-clean; RTL Arabic-first; ≥44×44 (SC-10, FR-16, P14) — `src/renderer/ui/catalogue/__tests__/CatalogueFreshness.test.tsx`
- [ ] T046 [US3] GREEN: `CatalogueFreshness` indicator + refresh affordance, reading `catalogue.freshness` and calling `catalogue.refresh`; truthful copy only — `src/renderer/ui/catalogue/CatalogueFreshness.tsx`
...
- [ ] T054 §A5 performance bring-up on target Windows hardware: read-down completion + promote-window at ~50k products; confirm 009's lookup budgets (NFR-1) hold against a read-down-populated catalogue — `specs/010-pos-catalog-read-down-consumption/perf-bringup.md`
