...
- [ ] **T291** [P] [US1] [DEFERRED → next slice w/ push primitive] Wire the banner mount into the renderer's persistent-banner host (per 003 / 007 banner-host pattern; layered on top of connection-state / operator-session banners per NFR-008) — `src/renderer/shell/AppShell.tsx` *(DEFERRED 2026-05-29, Ahmed: blocked on the renderer push primitive — `sales.subscribe(recent|banner_state)` is the `not_implemented` stub, so AppShell has no "which sale just finalized" feed to source `printFailure`. The component is built + tested; the AppShell mount + live feed land NEXT SLICE alongside the push primitive (`webContents.send` + token registry). Path corrected: the `BannerHost.tsx` named here does NOT exist — banners mount in `AppShell.tsx`. See coordination.md S3c preflight #2.)*
