...
**Status:** DESIGN-READY, IMPLEMENTATION-BLOCKED — `plan.md` + `tasks.md` are now authored (design-complete; see [./plan.md](./plan.md), [./tasks.md](./tasks.md), [./BLOCKER.md](./BLOCKER.md)), but **implementation stays BLOCKED**. **gated — requires owner approval + G10 verification before any dispatch.** **Date:** 2026-06-11. **Last Updated:** 2026-06-13. **Owning repo:** POS-Pulse. **Deciders:** Owner (Ahmed Shaaban).

> **Implementation blocker (verified 2026-06-13 on DP-2 `origin/main`).** The provider-neutral §16 `user_id` is **never delivered to the terminal** — a NEW upstream Data-Pulse-2 slice must surface it on a POS-facing sign-in/envelope response before any implementation can begin. DP-2 ships only `PosOperatorSessionSummary = { id, issued_at, envelope }` (an opaque bearer envelope + the Clerk-subject `id` = `clerk_user_id`); zero POS-facing contracts carry `user_id`; `IdentityProviderPort` resolves the §16 link server-side only. **G10** is satisfied-for-boundary-decisions (not yet ratified). **D3**'s `external_identity_links` table exists server-internally (DP-2 #550) but its provisioning is DEFERRED and it does **not** surface `user_id` to the terminal. **016 (D5) shipped/merged** but delivers the opaque envelope + `clerk_user_id`, NOT `user_id`. All 20 tasks are `[BLOCKED: user_id-delivery]`; no migration SQL is authored; `cashier_pin_records` is untouched. This is a tracked upstream dependency, not a spec defect. The DOUBLE-GATE, the PIN-locality invariant (E-4: `pin_hash`/`pin_salt` never leave the device), and the E-1..E-4 citations below are unchanged.
