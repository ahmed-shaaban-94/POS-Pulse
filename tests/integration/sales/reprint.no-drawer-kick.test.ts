/**
 * T404 — reprint does NOT kick the drawer (RED).
 *
 * A reprint of a cash-inclusive sale must NOT issue a DK1/DK2 pulse and must
 * NOT INSERT a second drawer_events row (FR-030 / AD-8). The reprint path wires
 * NO drawerKickDispatcher, so the kick is structurally impossible; this test
 * asserts the observable consequence — no fresh drawer_events row appears for
 * the sale after a reprint. The UNIQUE(sale_id) constraint on drawer_events
 * (migration 0023) would reject a second INSERT anyway; this asserts the
 * application code never attempts one.
 */
...
function makeBridge(db: SqlJsDatabase) {
  ...
  // Deliberately NO drawerKickDispatcher — reprint must never kick.
...
describe('T404 — reprint of a cash-inclusive sale does not kick the drawer', () => {
  ...
  it('no drawer_events row is written for the sale after a reprint', async () => {
    const bridge = makeBridge(db);
    const drawerRepo = bindDrawerEventsRepository(makeSqlJsHandle(db));

    expect(drawerRepo.readBySale('sale-1')).toBeNull();
