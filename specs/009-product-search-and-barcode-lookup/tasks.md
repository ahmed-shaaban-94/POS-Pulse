...
- [X] T049b [US1] **(added 2026-05-31 — T049a out-of-scope seed prerequisite)** Dev-only fixture seed for `products`/`product_barcodes` so the live happy-path (scan/search→confirm→add) and the S5 review tasks can exercise real rows. — `src/main/catalogue/dev-seed-catalogue.ts` (new), `src/main/index.ts` — *as-built (spec/plan `2026-05-31-009-t049b-*`; subagent-driven, 3 tasks): `applyDevSeedCatalogueIfRequested({ isPackaged, env, db, logger })` mirrors `applyDevSkip*IfRequested` — **fail-closed**: `isPackaged === true` short-circuits unconditionally (env var never consulted in a packaged build), else requires `POS_PULSE_DEV_SEED_CATALOGUE` truthy, else no-ops if `products` already populated (idempotent; `INSERT OR IGNORE` belt-and-suspenders). ~12 realistic Egyptian-pharmacy products on **`dev-tenant`** (matches the dev operator fixture's tenant — P17-visible) covering Arabic+English names, aliases (FR-13), a controlled + an Rx + an inactive product (C1 badges / disabled path), and a pack+unit barcode pair. `*_norm`/`*_fold` derived via the REAL `normalize()` (never hand-typed). One boot call site in `index.ts` post-migrations, next to the dev bypasses. Tests prove a seeded product is findable via `ProductRepo.search` + a barcode via `lookupByBarcode` (feeds the production read path, not just row presence). **Unblocks T050 + T056.** Out of scope: production seeding (platform's job; 009 is read-only, AD-2), any renderer/bridge change.*
- [X] T050 [US1] Screenshot/contact-sheet review of the live surfaces against S0; record consistency fixes — `specs/009-product-search-and-barcode-lookup/visual-direction/s5-screenshot-review.md` — *as-built (textual as-built review per the S0/T049a convention; this env can't screenshot a running Electron window): read every surface component + the live composition root, compared surface-by-surface to the contact sheet. Per-component fidelity PASS; found **two live-composition gaps** the FSM-only tests missed — **F1 (HIGH):** `CatalogueSalePane` reached `not_found`/`ambiguous`/`catalogue_unavailable` but mounted no surface for them (blank screen — breaks SC-10/FR-6/7/24); **F2 (minor a11y):** no `searching` busy affordance. Both **surfaced to owner as a scope decision** (real test-first code, not a review edit) → owner chose "fix now". Closed test-first in the same change: added the 3 error-state branches to `CatalogueSalePane` with `onEdit`→`recoverToInput` (`clear()` **+ focus-return** — `ProductSearchInput` gained an imperative `focus()` handle), the not-found surface now **echoes the searched value** (widened the `not_found` FSM variant to carry `query`), plus a `role=status`/`aria-busy` searching line (F2); 8 RED→GREEN composition tests. Renderer suite 1036 ✅, typecheck/lint/prettier ✅. Nits F3 (always-on hint)/F4 (`⊘` vs documented "none")/F5 (stale `ProductConfirmPanel` comment) left unfixed. T051 still owns the end-to-end axe sweep.*
- [X] T051 [P] axe-clean across all states (idle/searching/results/not-found/catalogue-unavailable/ambiguous/confirm) — `src/renderer/ui/catalogue/__tests__/a11y.full.test.tsx` — *as-built (parallel subagent): the **live-composition** axe sweep, distinct from T019's per-component isolation sweep. Renders the real `CatalogueSalePane`, drives `catalogueSearchStore` through all 7 states (each waited on the actual rendered DOM surface, not just `state.kind`), and asserts `expectNoAxeViolations(container)` per state — proving the assembled screen (cross-surface ids/landmarks) is clean, not just the parts. Reuses CatalogueSalePane.test.tsx's bridge-injection + fireEvent patterns. 7/7 green; **no violations found**, no source fix or disabled rule needed. Builds on the T050 fix (all 7 surfaces now mount live).*
- [X] T052 [P] Author the support runbook + failure-mode catalogue — `docs/runbook/product-search.md` **(§A5)** — *as-built (parallel subagent): mirrors the 008 runbook (header/§P15/Status-note/symptom→diagnosis→action). Covers product-not-found incl. inactive (FR-18), ambiguous barcode (FR-7 → upstream dedup, not a POS fix), catalogue-unavailable (FR-24, distinct from not-found, NOT the offline banner), generic add failure (FR-19/22), signed-out gate (NFR-6a), staleness-invisible (FR-24a). Schema/columns verified against `migrations/0029`–`0030` + `product-repo.ts` (not invented). **Honest finding:** there is no production log site for the diagnostic reasons (catalogue repo swallows read failures generically; logging in `src/main/catalogue/` is test-only) — so diagnosis is framed as **direct read-model inspection**, with a "note on logs" stating support should not expect a named reason field.*
- [X] T053 [P] Author the rollback strategy note (forward-fix preferred; read-only schema is low-risk) — `specs/009-product-search-and-barcode-lookup/rollback.md` **(§A5)** — *as-built (parallel subagent): §1 forward-fix posture, §2 three levers (flag-flip / code-revert / table-drop), §3 low-risk rationale, §4 out-of-scope, §5 decision table. Verified facts: `productSearch` is a **runtime** env var (`POS_PULSE_FEATURE_PRODUCT_SEARCH`) but read **once at boot** → flip takes effect next launch, not instant. Migrations `0029`/`0030` are additive-only (CREATE … IF NOT EXISTS, zero rows, no ALTER). **Corrected the task line's oversimplification:** the flag gates only the renderer UI; the R7 resolver is wired into 005's `cart-bridge.ts` **unconditionally** in main — so a flag-flip leaves the (inert) resolver installed, and only a **code-revert** hits 005's `DEFAULT_ITEM_REF_RESOLVER` generic-refusal fallback. Honest framing: disabling returns 005 to pre-009 behaviour (no add-by-lookup), not "cart fully functional".*
- [X] T054 Performance bring-up on target Windows hardware @ ~50k-row fixture; record NFR-1/NFR-2 evidence; if search misses ≤150 ms, trigger R4 FTS5-fallback review (R-RISK-1) — `specs/009-product-search-and-barcode-lookup/perf-bringup.md` **(§A5)** — ***scaffold authored 2026-06-01** ([`perf-bringup.md`](./perf-bringup.md)): turn-key harness + recording template — methodology, ≥50k on-disk seed recipe (real migration runner, production `better-sqlite3` NOT sql.js, indexes `0029`/`0030` confirmed), p95 protocol (warm-up discard + N≥1000 monotonic samples, full min/p50/p95/max), scenario tables, and the R-RISK-1/FTS5 decision gate all pre-filled. **Plus a runnable harness** [`scripts/perf-seed-catalogue.ts`](../../scripts/perf-seed-catalogue.ts) (`npm run perf:seed`): seeds a ≥50k on-disk db through the production seams (`openDatabase`→migration runner→`createProductRepo`), real `normalize()` folds, confirms `0029`/`0030` indexes, and `--measure` prints min/p50/p95/max per NFR-1/NFR-2 scenario. Lint+typecheck clean; smoke-validated through the real `openDatabase` seam (stops only at the expected Electron-vs-system-node `better-sqlite3` ABI boundary — owner runs `npm rebuild better-sqlite3` first per the doc's ABI note). The script **prints** numbers, never self-fills the doc. **RUN on target hardware 2026-06-01** (owner-supplied, LENOVO 82K2 / Ryzen 7 5800H / NVMe SSD, SHA `3461f00`, two 1000-sample runs within noise): **NFR-1 PASS** (worst p95 0.160 ms ≤ 50) · **NFR-2 PASS** (worst p95 15.170 ms ≤ 150) · **R-RISK-1/FTS5 review NOT triggered** — evidence in [`perf-bringup.md`](./perf-bringup.md) §2/§5/§6. Arabic-folded *timing* scenario not emitted by the harness (recorded, not fabricated; fold *correctness* covered by SC-9). **§A5 sign-off remains OWNER-GATED — not done.***
- [X] T055 Update the `<!-- SPECKIT START -->` active-feature block in `CLAUDE.md` to reflect 009 implementation status — `CLAUDE.md` — *as-built: made 009 the **Active feature** with gate + slice + PR granularity (§A0/§A1/§A2 ratified, **§A5 OPEN**; S0–S4 merged #317–#328, T049a/b #327/#328; S5 — T050 #330, T051–T053 #331 all merged; T054/T056–T058 remaining). States "009 not complete" — perf bring-up + §A5 sign-off remain. Demoted **008** to Recently-closed preserving its load-bearing caveat (§A5 signed off for internal/dev MVP only — Egyptian VAT deferred, customer-facing fiscal use BLOCKED pending 008-v2). Edit confined to the `<!-- SPECKIT START -->…END -->` markers; the adjacent "Spec Kit workflow" stale parentheticals left untouched (not T055's scope). **Finish-pass refresh 2026-06-01 (post-#334):** re-synced the SPECKIT block to mark T056 (#333) + T057/T058 (#334) merged and the entire CI/automated tier complete — only T054 (perf @ target hardware) + §A5 owner sign-off (+ manual legs of T050/T056) now remain; tightened the §A5 + intro lines accordingly.*

## Phase Final — Polish & cross-cutting

- [X] T056 [P] Full keyboard-only walkthrough of every story (scan→confirm→add, search→select→add, duplicate-scan) with no mouse (SC-1) — manual + `src/renderer/ui/catalogue/__tests__/keyboard-walkthrough.test.tsx` — *as-built (automated leg): drives every story end-to-end through the **live** `CatalogueSalePane` with keyboard only (`user.keyboard`/`user.type`, never `click`, never store-poke), asserting **focus position** at each transition, not just `state.kind`. RED-tested first → surfaced a real **SC-1 gap** (owner: "fix now"): the confirm panel had **no keyboard focus management** — didn't move focus into the `aria-modal` dialog on open, no Escape→cancel, no focus-return to the input after Add (3/4 stories stranded a keyboard user). **Fixed test-first:** `ProductConfirmPanel` now auto-focuses the Add button on open + handles Escape→`onCancel`; `CatalogueAddController` gained `onResolved` (fired on add-success AND cancel, NOT on refusal); `CatalogueSalePane` wires `onResolved`→`searchInputRef.focus()`. 4/4 walkthrough stories green; catalogue suite 109 ✅; typecheck/lint/prettier ✅. **Manual leg (human, packaged build, no mouse) cannot run in jsdom — owner-gated, deferred** (same constraint as T050 screenshots).*
- [X] T057 [P] Coverage-gate verification: ≥95% `catalogue-bridge.ts`, ≥95% `normalize.ts`, ≥90% `catalogueSearchStore` (Constitution VI) — CI report — *verified 2026-06-01 via `npx vitest run --coverage` (single run — `npm test --` drops the flag on this machine; never two concurrent coverage runs). All three target modules at **100%** lines/funcs/branches, exceeding every gate: `src/main/catalogue/catalogue-bridge.ts` 100% (35/35 ln, 7/7 fn, 23/23 br); `src/main/catalogue/normalize.ts` 100% (9/9 ln, 2/2 fn, 0/0 br — pure transforms, no branch points, satisfied by empty-set); `src/renderer/stores/catalogueSearchStore.ts` 100% (29/29 ln, 20/20 fn, 16/16 br). Whole-repo overall 96.05% stmt / 92.67% br / 98.16% fn / 97.86% ln. **Path note:** the as-built files are `catalogue/normalize.ts` and `stores/catalogueSearchStore.ts` (camelCase), not the kebab paths the task line implies.*
- [X] T058 Final `npm run codegen:verify && npm run typecheck && npm run lint && npm test` green with the full 009 diff — CI — *verified 2026-06-01, all four legs exit 0: `codegen:verify` (`api-types.ts` up to date); `typecheck` (all 3 tsconfigs — renderer/main/preload); `lint` (eslint clean + prettier "All matched files use Prettier code style"); `test` (full Vitest suite green in the T057 coverage pass).*
