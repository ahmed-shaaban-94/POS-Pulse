...
- [ ] T089 [US3] Implement `operator.forceCloseShift` in `src/main/operator/forced-close-handler.ts`: requireRole `manager` or `admin`; verifies the calling session's `branch_id` matches the stuck shift's `branch_id` (P17); writes shift's `lifecycle_state = 'closed_forced'` and `declared_count = null` (the absent state); emits `shift.forced_close` audit event with both identities and the structured reason — depends on T015, T046. **Implementation-time open question (see [`./planning/s5-speckit-readout.md`](./planning/s5-speckit-readout.md) §3.1):** stuck-shift discovery across terminals — resolve before this task begins (Wave 4.1 endpoint vs existing-sync verification vs client-side reconstruction).
- [ ] T090 [US3] Implement `src/renderer/ui/operator/ForcedCloseSurface.tsx` (Surface 4A — stuck-shift list + Surface 4B — forced-close form). **Note 3 (Slice 0) acceptance: default to card-stack rendering for the row list per visual-direction reviewer lean; revisit only if integration-time density evidence argues otherwise.** — depends on T017, T076
