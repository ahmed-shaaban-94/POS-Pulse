...
| **§A3** — Migrations | ✅ **CLEARED 2026-05-27** — Ahmed signed migration review `approved` (PR #256); ten-item scope checklist verified. Slice 1 §A3 migration tasks T020–T027 authorized. See `coordination.md` §"§A3 migration reviewer thread (T003)". | (gate now cleared; Slice 1 persistence unblocked) |
| **§A4** — Bridge-API surface | ✅ **CLEARED 2026-05-27** — Ahmed signed bridge-API security review `approved` (PR #257); eight-item §A4 security checklist verified. Slice 1 bridge handlers T100/T101 + all later slices' bridge work authorized. See `coordination.md` §"§A4 bridge-API reviewer thread (T004)". | (gate now cleared; Slice 1/2/3/5/6 bridge work unblocked) |
...
- [ ] **T052** [P] [US1] *(revised twice — v1 R1 EventEmitter→update_hook, v2 CR1 update_hook→periodic scan worker. AD-2 v3 LOCKED 2026-05-27.)* Test (failing): the AD-2 v3 scan worker registers a `setInterval` (default 200 ms; configurable) that runs the canonical `SELECT … FROM audit_events WHERE category='payment.settled' AND terminal_id=? AND NOT EXISTS (SELECT 1 FROM sales WHERE envelope_handoff_action_id = json_extract(payload_json, '$.handoff_action_id')) ORDER BY created_at ASC LIMIT 32` (per plan §AD-2). For each returned row, dispatch AD-2 finalize with the row's `handoff_action_id` as the idempotency key. Assert: (a) no `payment.settled` row is double-finalized across concurrent worker ticks (the `NOT EXISTS` clause guarantees this); (b) a row finalized by a previous tick does NOT appear in subsequent scans; (c) the scan is bounded by `LIMIT 32` per tick; (d) the worker stops the next tick from running before the current tick's finalize work completes (single-flight per tick). — `tests/unit/main/sales/finalize-listener.scan-worker.test.ts`
...
- [ ] **T090** [US1] *(revised twice — v1 R1, v2 CR1; AD-2 v3 polling LOCKED)* Implement the AD-2 v3 scan worker (`src/main/sales/finalize-listener.ts`): at main-process startup, register a `setInterval` worker against the shared better-sqlite3 connection; default tick = 200 ms (configurable via terminal config; floor 100 ms, ceiling 1000 ms). Each tick runs the canonical `SELECT` from plan §AD-2 (filtered by `category='payment.settled'`, the current paired terminal's `terminal_id`, and `NOT EXISTS` Sale row), then dispatches AD-2 finalize for each returned `handoff_action_id`. Single-flight per tick (do not start tick N+1 until tick N's dispatched finalizes have returned or timed out). **No `db.updateHook` call** (does not exist in better-sqlite3 ^12.9.0). **No `EventEmitter` import from 006.** **No `CREATE TRIGGER` migration row.** Polling only. — `src/main/sales/finalize-listener.ts`
...
- [ ] **T092** [US1] *(revised twice — v1 R1, v2 CR1; AD-2 v3 makes "startup recovery" identical to "first tick of the steady-state scan", so this task now covers the **adjacent** print/drawer recovery sub-scans per research §R-15 v3)* Implement two **one-shot startup recovery sub-scans** in `src/main/sales/finalize-listener.ts`, run **once** at main-process startup (not periodically — they're one-shot recovery, not steady-state mechanisms): (1) **Print recovery:** scan `sales` for any sale whose `print_events` table has no `outcome='success'` row AND no `outcome='manual_override'` row; dispatch a fresh print attempt per match. (2) **Drawer recovery:** scan `sales` for any cash-inclusive sale (`tender_lines_summary_json` contains an applied `cash` line) whose `drawer_events` table has no row; dispatch a fresh drawer-kick attempt per match. Both scoped to the current terminal's `(tenant_id, branch_id, terminal_id)` triple. Both complete in a single pass and do not re-run. The audit-events recovery is NOT a separate task — it's automatic via the T090 steady-state worker's first tick. — `src/main/sales/finalize-listener.ts`
