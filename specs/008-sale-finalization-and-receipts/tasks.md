...
- [ ] **T512** [US1] [IMPECCABLE craft] Wire the Manual receipt override button on `<PrinterFailureBanner>` to invoke `receipts.manualOverride` with a fresh idempotency key per click; on success, dismiss the banner. **FUNCTIONAL CORE DONE, CRAFT GATE OPEN:** the Manual button now calls `receipts.manualOverride` directly (mirroring Retry — local in-flight phase; the `onManualOverride` entry-point prop was removed from `<PrinterFailureBanner>` + AppShell; dismiss is automatic via the banner_state projection since the override row supersedes the failure). Satisfies the T512 affordance test. **STILL OPEN before Slice 6 merge:** the mandatory `/impeccable craft 008-printer-failure-banner-manual-override` polish pass + §A1 red-bar record (per `docs/impeccable-embed-preflight.md §4.2`) were NOT performed — left unchecked so the tracker reflects the open craft gate. — `src/renderer/ui/receipts/PrinterFailureBanner.tsx`
...
- [ ] **T528** [§A5] — **MOSTLY VERIFIED, one sub-item owner-gated (2026-05-30).** typecheck ✅, lint ✅, full `npm test` ✅, `codegen:verify` no-op ✅ (all local + a5-verification-findings.md §T528). `package:dir` is a `windows-latest` CI-only step: the last **fully-green `main` CI** run was `7fed614` (#307); the #308/#309 runs failed only on the known self-hosted `cancelled`/`ENOTEMPTY` flake (tests passed in-log), and #308/#309 are **docs-only** (cannot affect `package:dir`). **Owner: confirm the latest `main` CI `package:dir` is green to close** — left unticked pending that confirmation (honest: not asserting a post-#309 green package:dir without a green run to point to). CI gates check: confirm `codegen:verify` passes as a no-op (AD-12), `typecheck` passes, `lint` passes, full `npm test` passes, `package:dir` smoke build passes on `windows-latest` — `specs/008-sale-finalization-and-receipts/coordination.md`
