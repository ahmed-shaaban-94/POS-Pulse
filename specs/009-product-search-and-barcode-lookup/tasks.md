...
- [ ] T054 Performance bring-up on target Windows hardware @ ~50k-row fixture; record NFR-1/NFR-2 evidence; if search misses ≤150 ms, trigger R4 FTS5-fallback review (R-RISK-1) — `specs/009-product-search-and-barcode-lookup/perf-bringup.md` **(§A5)** — ***scaffold authored 2026-06-01** ([`perf-bringup.md`](./perf-bringup.md)): turn-key harness + recording template — methodology, ≥50k on-disk seed recipe (real migration runner, production `better-sqlite3` NOT sql.js, indexes `0029`/`0030` confirmed), p95 protocol (warm-up discard + N≥1000 monotonic samples, full min/p50/p95/max), scenario tables, and the R-RISK-1/FTS5 decision gate all pre-filled. **Plus a runnable harness** [`scripts/perf-seed-catalogue.ts`](../../scripts/perf-seed-catalogue.ts) (`npm run perf:seed`): seeds a ≥50k on-disk db through the production seams (`openDatabase`→migration runner→`createProductRepo`), real `normalize()` folds, confirms `0029`/`0030` indexes, and `--measure` prints min/p50/p95/max per NFR-1/NFR-2 scenario. Lint+typecheck clean; smoke-validated through the real `openDatabase` seam (stops only at the expected Electron-vs-system-node `better-sqlite3` ABI boundary — owner runs `npm rebuild better-sqlite3` first per the doc's ABI note). The script **prints** numbers, never self-fills the doc. **Numbers/verdict/hardware spec left `[TBD — target hardware]` — task stays `[ ]`; no CI substitute, owner runs on the real terminal.** Still OPEN: the actual bring-up run + §A5 sign-off.*
