...
- [ ] T043 [US3] GREEN: register the catalogue channels via `ipcMain.handle` + wire in `index.ts`. **DEFERRED — split by dependency (corrected 2026-06-07):**
  - **The whole catalogue surface is currently INERT at runtime.** 009 never wired ANY `catalogue.*` channel to `ipcMain` (verified firsthand: no registrar, no loop, no `ipcMain.handle('catalogue:…')` anywhere in `src/main/`). The renderer preload invokes channels with no main-side handler. So 009's four read handlers AND 010's `refresh`/`freshness` are all unreachable from the renderer until this registration lands. The bridge factory + preload + UI are complete and unit-tested; none is wired.
  - **`catalogue:freshness` is NOT #349-blocked** — it needs only the shipped `catalogue-sync-state-repo` + a tenant-scoped product count. The count is the one missing piece: `productRepo` exposes `lookupByBarcode`/`lookupBySku`/`search`/`resolveForSeam` but NO count, so a `CatalogueFreshnessSource` adapter (sync-state read + a `SELECT COUNT(*) … WHERE tenant_id=?`) must be authored. This is **in-scope-now work**, not a #349 deferral.
  - **`catalogue:refresh`** needs the read-down driver, which needs the live client + `pairingStore` scope (T039) — **that** leg is #349-blocked.
  - **Post-wiring re-check of AD-1/SEC-1 required against the live `ipcMain` handler (see s4-review §10).** Original: register the two new channels — `src/main/ipc/catalogue.ts` (+ wire in `src/main/index.ts`)
