...
- `specs/017-offline-pin-reanchor` — POS Offline-PIN Re-Anchor off a DP-2 provider-neutral identifier (drift **D6**). **DESIGN-READY, IMPLEMENTATION-BLOCKED — sole remaining blocker is owner Queue dispatch (no longer technical).** Re-keys `cashier_pin_records`'s composite PRIMARY KEY from the provider-coupled `cashier_clerk_user_id` onto the §16 provider-neutral `user_id`; PIN secret never moves; `clerk_user_id` → nullable bridge column. Full speckit chain authored (PR #385, `a6d20bc`): `/speckit-plan` ✅ (design-complete SQLite PK-rebuild; Constitution v1.5.1 check PASS, VIII advanced) · `/speckit-tasks` ✅ (20 tasks, **all `[BLOCKED: owner-dispatch]`** — reclassified from `[BLOCKED: user_id-delivery]`) · `/speckit-analyze` ✅ (0 CRITICAL, 100% coverage, `implementationBlocked: true`). **The double-gate is now resolved down to one governance act (see [`BLOCKER.md`](specs/017-offline-pin-reanchor/BLOCKER.md), reconciled to the Orchestrator kernel):** **(gate-1, `user_id`-delivery) ✅ SATISFIED** — DP-2 shipped **033** (`feat(033)`, PR #567, `c5e1c5d`) adding the provider-neutral §16 `user_id` (= DP-2 `users.id`) as an additive, `required`, `uuid` sibling field on `PosOperatorSummary` (present on sign-in, manager/admin, takeover-confirm incl. idempotent replay; NOT in the opaque envelope), and POS **consumes** it via **PR #389 (`75f5e6d`)** — `user_id` added to `BackendSignInOperator` and read through in `interpretSignInResponse` (`backend-client.ts:407`; required, fails closed to `refused` if absent). POS validates the operator path **leniently** (allowlist reader, no zod/ajv — INBOX-DP2-033 answered), so the additive field was always wire-safe and no codegen re-pin was needed (the operator contract is hand-mirrored, not in `api-types.ts`). **(gate-2a, G10) ✅ SATISFIED** — the Orchestrator kernel has **no `ratified` G10 state**; the node status `satisfied-for-boundary-decisions` IS the satisfied state for all consumers (required evidence met: ORCH-028 #85 `76cfcc3`; OQ-1/5/6/7/8/10 signed). The earlier "G10 must be ratified" phrasing was a POS-017-local invention stricter than the kernel; corrected in PR #391. **(gate-2b, owner dispatch) ❌ REMAINING — the ONLY blocker:** D6 becomes a POS-Pulse Queue Item only with explicit, scoped **owner approval** to mint/dispatch POS-017 — a governance decision (heaviest item in the 028 drift set: a SQLite `cashier_pin_records` PK-rebuild on offline records), external to this repo's code. **Until dispatched: no implementation, no migration SQL, `cashier_pin_records` untouched.** The design executes as-is the moment the owner dispatches it.
