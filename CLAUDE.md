...
**010 slice status:** **NONE merged — pre-implementation.** Slice plan (from [plan.md](specs/010-pos-catalog-read-down-consumption/plan.md) §Phase 2): S0 re-pin `api-types.ts` (§A6, **DEFERRED** — blocked on backend deploy, issue #349) → S1 migrations `0031`–`0033` (§A2 ✅; **buildable now**) → S2 read-down writer + validation (§A2 ✅; **buildable now**) → S3 HTTP client + driver (§A6 — waits on the re-pin; `Authorization: Bearer <device_token>` auth) → S4 bridge additions + freshness UI (§A4 lane open) → S5 production readiness (§A5). The correctness core (migrations, writer/promote/atomicity, validation, fold-parity, tenant-isolation) is **§A2-ratified and buildable offline now**; only the live-fetch client (T020/T021) + wiring (T039) truly wait on §A6's D-DEPLOY (the live site at `api.smartdatapulse.tech` was returning HTTP 521 — owner decision: work locally meanwhile). The pinned `api-types.ts` is **still the stale** `/api/v1/pos/catalog/products` shape until the re-pin.
