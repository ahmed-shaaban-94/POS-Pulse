...
| _OBSERVED (not promoted to tested)_ — **BIXOLON SRP-330 II** | USB (OS print queue); ESC/POS-capable thermal printer (direct path not yet exercised) | Vendor driver **installed**; ESC/POS direct command set expected but unverified on this unit. **Best observed driver paper setting: 80 × 3276 mm continuous roll** (see caveats) | 008 §A5 bench + **T301 OS-print bench** (observed 2026-05-30; see [specs/008-sale-finalization-and-receipts/coordination.md](../specs/008-sale-finalization-and-receipts/coordination.md) §"T301 OS-print bench result") — **not promoted to a tested row** (rule-1 integration test still PENDING); ESC/POS direct path still unexercised | Windows **OS test-page printed successfully**. A **browser/HTML receipt-template smoke** passed. **✅ Official OS-print-pipeline smoke PASSED (T301, 2026-05-30, PR #304):** a real 008 receipt printed from the official POS-Pulse OS-print pipeline (`webContents.print` via a secure offscreen window). **Render quality:** Arabic legible, English legible, **no** card/voucher data on the slip, **no** excess blank paper, feed/cut acceptable, darkness acceptable; **edge clipping resolved at a 70 mm printable body width** inside the 80 mm roll (`pageSize` stays 80 mm). **ESC/POS direct path NOT yet verified** (OS-print is the selected, proven path). **Paper setting:** best observed driver setting is **80 × 3276 mm continuous roll** — short fixed forms such as 80 × 287 mm may feed excessive blank paper. **Owner decision 2026-05-30 (Ahmed): this is the ACCEPTED 008 §A5 printer target (Option A)** — the §A3-committed Epson TM-T20III pair is superseded for the 008 MVP. **ESC/POS is descoped; OS-print is the accepted path.** Promotion to a *tested* row is STILL OPEN — it needs the rule-1 integration evidence, and whether the bench smoke clears that bar is an owner-accepts call (bar-question pending; see coordination.md §"Owner decision — 008 §A5 hardware target (2026-05-30)"). |
