...
- [ ] T043 [BLOCKED:D-DEPLOY #349 / T039] [US3] GREEN: register the two new channels via `ipcMain.handle` + wire in `index.ts`. **DEFERRED** — 009 never wired the catalogue bridge to `ipcMain` either, and the driver wiring needs `pairingStore` scope + the #349-blocked live client (T039). The bridge factory + preload are done + unit-tested; registration lands with T039. **Post-wiring re-check of AD-1/SEC-1 required (see s4-review §10).** Original: register the two new channels — `src/main/ipc/catalogue.ts` (+ wire in `src/main/index.ts`)
