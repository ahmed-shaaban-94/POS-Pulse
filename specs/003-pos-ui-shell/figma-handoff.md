...
|:---|:---|:---|:---|
| `surface` | `--color-surface` | `#ffffff` | Primary app surface |
| `surface-muted` | `--color-surface-muted` | `#f5f5f5` | NavRail, secondary surfaces |
| `surface-elevated` | `--color-surface-elevated` | `#ffffff` | Cards, dialogs |
| `text` | `--color-text` | `#111827` | Default body text |
| `text-muted` | `--color-text-muted` | `#6b7280` | Secondary text, labels |
| `text-inverse` | `--color-text-inverse` | `#ffffff` | Text on emphasis surfaces |
| `primary` | `--color-primary` | `#2563eb` | Primary action |
| `primary-emphasis` | `--color-primary-emphasis` | `#1d4ed8` | Primary hover/active |
| `primary-on` | `--color-primary-on` | `#ffffff` | Text on primary |
| `danger` | `--color-danger` | `#dc2626` | Destructive, error |
| `danger-emphasis` | `--color-danger-emphasis` | `#b91c1c` | Danger hover/active |
| `danger-on` | `--color-danger-on` | `#ffffff` | Text on danger |
| `warning` | `--color-warning` | `#d97706` | Warning |
| `warning-emphasis` | `--color-warning-emphasis` | `#b45309` | Warning hover/active |
| `warning-on` | `--color-warning-on` | `#ffffff` | Text on warning |
| `success` | `--color-success` | `#059669` | Success, online state |
| `success-emphasis` | `--color-success-emphasis` | `#047857` | Success hover/active |
| `success-on` | `--color-success-on` | `#ffffff` | Text on success |
| `neutral` | `--color-neutral` | `#4b5563` | Neutral, syncing state |
| `neutral-emphasis` | `--color-neutral-emphasis` | `#374151` | Neutral hover/active |
| `neutral-on` | `--color-neutral-on` | `#ffffff` | Text on neutral |
| `focus-ring` | `--color-focus-ring` | `#2563eb` | Visible focus outline |
| `border` | `--color-border` | `#e5e7eb` | Default borders |
...
|:---|:---|:---|
| `family.sans` | `--font-family-sans` | `ui-sans-serif, system-ui, -apple-system, sans-serif` |
| `family.mono` | `--font-family-mono` | `ui-monospace, 'Cascadia Code', monospace` |
...
| State | Intent Class | Indicator color | Text label | Banner |
|:---|:---|:---|:---|:---|
| `online` | `--success` | `--color-success` | "Online" | hidden |
| `degraded` | `--warning` | `--color-warning-emphasis` | "Connection slow" | warning banner |
| `offline` | `--danger` | `--color-danger` | "Offline" | danger banner |
| `syncing` | `--neutral` | `--color-neutral` | "Syncing…" | neutral banner |

Each state is distinguished by: color + text label + status dot shape.
Connection indicators are NEVER color-alone — text label is always visible.
