...
| `payment.force_failed` *(Slice 4)* | `payments.forceFail` succeeds | `payment_attempt_id`, `failure_reason='force_failed_by_manager'`, `attribution_operator_id` (the cashier whose attempt was force-failed) + `force_fail_attribution_operator_id` (the manager). The manager identity lives in the audit payload, never in the renderer-visible UI. |
