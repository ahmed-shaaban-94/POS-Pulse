...
**Created:** 2026-06-04
**Status:** **S1+S2 correctness core MERGED** — commit `5895eba` (`feat(010): offline read-down S1+S2 correctness core (migrations + writer)`) shipped the migrations `0031`–`0033`, the read-down writer (stage+promote), `mapSellableRow`, `validateRecord`, the `catalogue_sync_state` repo, and 13 test files (**64 tests green**, verified 2026-06-07). Tasks ticked below reflect that commit. Gates §A4 / §A5 remain; **§A6 is NARROWED to D-DEPLOY only** (see below). **Update 2026-06-07 (`feat/010-driver-bridge-ui`):** the driver (T037/T038), no-outbound-write (T036), read-down redaction smoke (T018), the bridge surface (T040/T041/T042/T044), the freshness UI (T045/T046), the post-impl §A4 walk (T050 — CLEARED), the polish docs (T051/T052/T053), coverage (T055) and final-green (T056) are all DONE. **Update 2026-06-07 (freshness wiring):** **T043 freshness leg DONE** — the `catalogue:*` IPC surface is now registered (`src/main/ipc/catalogue.ts` + `index.ts`); 009's lookups + 010's `catalogue:freshness` are reachable from the renderer (`productRepo.countByTenant` added; `CatalogueFreshness` mounted in `CatalogueSalePane`; §A4 post-wiring re-check CLEARED, s4-review §11). **Remaining open:** **T002/T020/T021/T039** (live HTTP client + read-down **driver** wiring — blocked on D-DEPLOY #349; `catalogue:refresh` is registered but refuses until the driver lands, needs a final §A4 `refresh` re-check) — **tracked in issue [#360](https://github.com/ahmed-shaaban-94/POS-Pulse/issues/360)** (blocked on #349; also carries the post-commit freshness-refresh follow-up, Codex-flagged, no-poll constraint) · **T054** (§A5 perf bring-up — needs target hardware) · **T057** ✅ DONE (CLAUDE.md slice status, owner-authorized 2026-06-07).
...
- [x] T018 [P] Extend the cross-process redaction allowlist + Sentry scrubber for any read-down secret-shaped field (append-only) and add a read-down redaction-smoke fixture — `src/shared/audit/forbidden-keys.ts` + `src/main/catalogue/read-down/__tests__/redaction.smoke.test.ts`
...
- [x] T036 [P] [US2] RED: no-outbound-write test — across the full read-down lifecycle the client issues only the snapshot GET; assert 0 other backend calls / 0 POS→backend writes (SC-7, FR-10) — `src/main/catalogue/read-down/__tests__/no-outbound-write.test.ts`
- [x] T037 [US2] RED: driver test — `runTickOnce` runs one read-down; single-flight (`already_running` while in progress); `start`/`stop` install/clear the interval; stop before db close — `src/main/catalogue/read-down/__tests__/read-down-driver.test.ts`
- [x] T038 [US2] GREEN: `createReadDownDriver({ client, writer, tickIntervalMs, tenantId, branchId, now })` → `{ runTickOnce, start, stop }`; async single-flight (admit sync, complete on `completed` promise); transport-failure path records a failed attempt without calling the writer (NOTE: `logger` dep dropped — read-down wires no logger, RED-1 vacuous; T018 pins the redaction invariant) — `src/main/catalogue/read-down/read-down-driver.ts`
- [ ] T039 [BLOCKED:D-DEPLOY #349 — tracked in [#360](https://github.com/ahmed-shaaban-94/POS-Pulse/issues/360)] [US2] Wire the driver into the composition root: instantiate `createReadDownClient` + `createReadDownWriter` + `createReadDownDriver`, injecting `tenant_id`/`branch_id` from **`pairingStore`** (not a session); run once on app-start / post-pairing; `start()` the interval; call `stop()` in `closeDbHandle()` **before** `dbHandle.close()` (mirror `finalize-listener` wiring) — `src/main/index.ts`
...
- [x] T040 [US3] RED: bridge contract test — `catalogue:refresh` → `started` / `already_running` / `refused(no_session|tenant_isolation)`; `catalogue:freshness` → `ok{last_success_at|null, is_empty}` / `refused`; assert the three freshness states (never-synced / synced-with-products / **synced-but-empty**, FR-16b/SC-10); both gate on session first; neither returns data/secret — `src/main/catalogue/__tests__/catalogue-bridge.readdown.test.ts`
- [x] T041 [US3] GREEN: add `CATALOGUE_IPC_CHANNELS.REFRESH` + `FRESHNESS` and the request/response types (freshness response carries `last_success_at` + `is_empty`) — `src/shared/catalogue/channels.ts` + `src/shared/bridge-api.ts`
- [x] T042 [US3] GREEN: extend `createCatalogueBridge` with `refresh` (session-gated → `driver.runTickOnce()`, single-flight status) and `freshness` (session-gated → `syncStateRepo` read + a tenant-scoped `products`-has-rows check for `is_empty`; tenant-scoped) — `src/main/catalogue/catalogue-bridge.ts`
- [x] T043 (freshness leg) GREEN — DONE 2026-06-07: register the catalogue channels via `ipcMain.handle` + wire in `index.ts`. **The whole catalogue surface was INERT** (009 shipped the bridge + preload but never registered ANY `catalogue:*` channel with `ipcMain` — verified firsthand). Now wired:
  - **`src/main/ipc/catalogue.ts`** (`registerCatalogueHandlers`) registers all six channels (009's 4 read + 010's refresh/freshness); thin delegation, generic-refuse on malformed payloads. Unit-tested.
  - **`src/main/index.ts`** constructs the bridge with `productRepo` + a `freshness` source (`catalogue-sync-state-repo.read` + `productRepo.countByTenant`) + an operator-session adapter (`getCurrent()` → `OperatorSessionForCatalogue`, mapping `id`→`operator_session_id`), and registers it. **This makes 009's lookups + 010's `freshness` reachable from the renderer.**
  - `productRepo.countByTenant(tenantId)` added (tenant-scoped, 0 on empty/unreadable) — the missing `is_empty` piece.
  - `CatalogueFreshness` mounted in `CatalogueSalePane`; `toState` hardened to degrade a malformed/undefined bridge response to `unavailable` (never white-screens).
  - **§A4 post-wiring AD-1/SEC-1/INP-1/P17-1 re-check DONE — CLEARED (s4-review §11).**
  - **STILL OPEN — the `refresh` leg:** the bridge is constructed with NO `readDownDriver` (refuses until the driver is wired), because the driver needs the live HTTP client + `pairingStore` scope (T039 / #349). A final §A4 `refresh` re-check is required when that lands.
- [x] T044 [US3] GREEN (§A4 lane open): expose `catalogue.refresh` / `catalogue.freshness` on the preload surface — `src/preload/*` (the typed `catalogue.*` exposure)
- [x] T045 [P] [US3] RED: freshness-indicator a11y/render test — shows the three FR-16b states ("not yet downloaded" when null; "last updated &lt;time&gt;" when non-empty; "updated &lt;time&gt; — no products available" when `is_empty`); no live-sync wording; keyboard-operable refresh affordance; axe-clean; RTL Arabic-first; ≥44×44 (SC-10, FR-16/16b, P14) — `src/renderer/ui/catalogue/__tests__/CatalogueFreshness.test.tsx`
- [x] T046 [US3] GREEN: `CatalogueFreshness` indicator + refresh affordance, reading `catalogue.freshness` (rendering all three `is_empty`/null states truthfully) and calling `catalogue.refresh`; truthful copy only. ABSOLUTE timestamp (owner: relative time needs a ticking clock or it lies); built via impeccable — `src/renderer/ui/catalogue/CatalogueFreshness.tsx`

---

## Phase Final — Polish & Cross-Cutting

- [x] T050 §A4 P8 bridge-security review of `catalogue:refresh` / `catalogue:freshness` — `specs/010-pos-catalog-read-down-consumption/security-review/s4-review.md`. Pre-impl lane opened `2fb565b`; **post-implementation walk DONE 2026-06-07 (§10): all 13 controls PASS (RED-1 deferred-vacuous), verdict CLEARED for the implemented bridge factory + preload + freshness UI.** Scoped exception: the `ipcMain.handle` registration (T043) re-opens AD-1/SEC-1 for a short post-wiring re-check before the rollout PR.
- [x] T051 [P] Support runbook entry (read-down failure modes, manual-refresh, "catalogue unavailable" triage) — `docs/runbook/catalogue-read-down.md`
- [x] T052 [P] Rollback note (disable the driver / revert migrations `0031`–`0033` / behaviour with an empty read model) — `specs/010-pos-catalog-read-down-consumption/rollback.md`
- [x] T053 [P] Failure-mode catalogue (transport / malformed / over-threshold / interrupted-promote / tenant-drift → operator-visible + diagnostic behaviour) — `specs/010-pos-catalog-read-down-consumption/failure-modes.md`
- [x] T054 §A5 performance bring-up on target Windows hardware — **DONE 2026-06-15, VERDICT PASS** (`perf-bringup.md` §7). NFR-1 (≤0.45 ms p95) / NFR-2 (≤48 ms p95) pass comfortably against a read-down-populated 50k catalogue. The SC-8 "concurrent lookup during promote" question resolved: production shares ONE synchronous main-thread connection (no worker threads), so a lookup CANNOT race the promote under WAL — it queues behind the writer's synchronous span. The honest SC-8 number is therefore the main-thread BLOCK = full `writer.run()` span, which was ~20 s @ 50k (autocommit-per-row staging). **Fixed (#411): wrap staging in one tx → ~3.6 s p50 (5.4×); accepted for v1** (background non-session-gated pull). Chunked R-RISK-2 declined for v1. — `specs/010-pos-catalog-read-down-consumption/perf-bringup.md`
